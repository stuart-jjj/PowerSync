...
from .const import (
    ...
    SENSOR_TYPE_SOLAR_FORECAST_TODAY_REMAINING,
    SENSOR_TYPE_SOLAR_FORECAST_TOMORROW,
...
LP_FORECAST_SENSORS: tuple[PowerSyncSensorEntityDescription, ...] = (
    ...
    PowerSyncSensorEntityDescription(
        key=SENSOR_TYPE_SOLAR_FORECAST_TODAY_REMAINING,
        name="Solar Forecast Today (Remaining)",
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        device_class=SensorDeviceClass.ENERGY,
        suggested_display_precision=1,
        icon="mdi:solar-power-variant",
        value_fn=lambda data: data.get("solar_today_remaining_kwh") if data and data.get("available") else None,
        attr_fn=lambda data: {
            "peak_kw": data.get("solar_peak_kw"),
        } if data and data.get("available") else {},
    ),
    PowerSyncSensorEntityDescription(
        key=SENSOR_TYPE_SOLAR_FORECAST_TOMORROW,
        name="Solar Forecast Tomorrow",
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        device_class=SensorDeviceClass.ENERGY,
        suggested_display_precision=1,
        icon="mdi:solar-power-variant-outline",
        value_fn=lambda data: data.get("solar_tomorrow_kwh") if data and data.get("available") else None,
        attr_fn=lambda data: {
            "peak_kw": data.get("solar_peak_kw"),
        } if data and data.get("available") else {},
    ),
